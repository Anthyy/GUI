...
public class MouseLook : MonoBehaviour 
{
    ...
    private void Update()
    {
        ...
        if(axis == RotationalAxis.MouseXandY)
        {
            ...
            float rotationX = transform.localEulerAngles.y + Input.GetAxis("Mouse X") * sensitivityX; // sensitivity = the speed at which we can move the camera
            ...
            rotationY = Mathf.Clamp(rotationY, minimumY, maximumY); // setting restrictions
...
public enum RotationalAxis
{
    MouseXandY,
    MouseX,
    MouseY // Cannot end with a semicolon anymore apparently
}

//Creating an enum out side the script (as in outside the class) and making it public means it can be asessed anywhere in any script with out reference
#endregion
